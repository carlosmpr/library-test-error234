...
- **dispatch**: A function created using `createEventDispatcher` from Svelte to dispatch events.
...
3. **extractFiles(items)**:
   ...
   - Validates each file and adds it to the filesArray if it is valid.

4. **traverseFileTree(item, filesArray)**:
   - Recursively traverses the file tree if a directory is dropped.
   - Resolves the promise when all files are added to the filesArray.

5. **isValidFile(file)**:
   - Checks if the file's extension is in the allowedExtensions array.
