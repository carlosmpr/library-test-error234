...
### HTML Structure
- A div element with drag and drop functionality for file upload.
- An input element of type file that is hidden and allows multiple file selection.
- A label element to trigger file selection when clicked.
- Displays selected files with their names if any, otherwise prompts to drag and drop files.

### Example Usage
```html
<!-- Example Usage of the Code -->
<script>
  import { DocumentTextIcon, ArrowUpTrayIcon } from 'some-icon-library';
</script>

<div>
  <!-- The code provided in the script tag -->
</div>
```

This code snippet allows users to drag and drop files or select files using an input element. It validates the files based on their extensions, limits the number of files that can be uploaded, and displays the selected files to the user.
